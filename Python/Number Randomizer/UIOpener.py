...
class MyWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        uic.loadUi(os.path.join(ui_path, "NumberRandomizer.ui"), self)
        self.initUI()
    ...
    def initUI(self):  # initialize
        ...
        self.TimeSpinBox.setValue(100) # to give people the chioce to go faster at thier oun risk
        self.StartBtn.clicked.connect(self.StartBtnFun)
        self.StopBtn.clicked.connect(self.StopBtnFun)
        self.HotkeyBtn.clicked.connect(self.HotkeyBtnFun)
        ...
        self.SelectAllBtn.clicked.connect(self.SelectAllfun)
        self.DeselectAllBtn.clicked.connect(self.DeselectAllfun)
        self.MouseListener = None  # for mouse listener
        self.RepeatCounter = 0  # Repat counter
        self.Settings = QSettings("NumberRandomizer", "NumberRandomizer")# to make settings file
    ...
    def StartBtnFun(self):  # start the app
        if not self.numbers:  # if list is empty
            ...
            self.StopBtn.setEnabled(True)  # enable stop button
            self.StartBtn.setEnabled(False)  # disable start button
    ...
    def SelectAllfun(self):  # select All
        ...
        buttons = [self.Btn1, self.Btn2, self.Btn3, self.Btn4, self.Btn5, self.Btn6,
                   self.Btn7, self.Btn8, self.Btn9]  # loop and disable all buttons
    ...
    def DeselectAllfun(self):  # deselect all
        ...
        buttons = [self.Btn1, self.Btn2, self.Btn3, self.Btn4, self.Btn5,
                   self.Btn6, self.Btn7, self.Btn8, self.Btn9]  # loop and enable all buttons
        for button in buttons:
            button.setEnabled(True)

    def mainfun(self):  # main function
        if self.OnClickRadioButton.isChecked():  # to check if checked no need to initialize
            self.OnClickRadioButtonCheckedfun()  # OnClick Radio Button Checked function
        elif self.TimeRadioButton.isChecked():  # to check if checked no need to initialize
            try:  # for globla hotkey
                self.hotkeyFun()  # this fix the disabling of the hotkey
            except Exception:
                pass
            while self.StopBtn.isEnabled():  # while app works
    ...
    def TimeRadioButtonCheckedfun(self):  # Time Radio Button Checked function
        ...
        QTimer.singleShot(self.TimeSpinBox.value(), loop.quit)  # sleep
        loop.exec_()  # sleep
        keyboard.press(RandomNumber)  # press random from list
        keyboard.release(RandomNumber)  # release random from list
        if self.RepeatRadioButton.isChecked():  # to check if checked no need to initialize
    ...
    def HotkeyBtnFun(self):
        if self.shortcut_edit.keySequence().isEmpty():  # if keySequenceEdit is empty
            QMessageBox.warning(
            ...
            try:  # try expect for hotkey length and special characters
                self.HotkeySequence = self.shortcut_edit.keySequence().toString().upper()[
                    0]
                ...
                # change button text to add the hotkey
                self.StopBtn.setText("Stop  ({})".format(self.HotkeySequence))
    ...
    def hotkeyFun(self):  # assign the globla hotkey and start listening
        if not self.shortcut_edit.keySequence().isEmpty():  # if keySequenceEdit is not empty
            self.HotkeyListener = keyboard.GlobalHotKeys({
                self.shortcut_edit.keySequence().toString(): self.OnHotkeyClick
    ...
    def OnHotkeyClick(self):  # what the hotkey dose when pressed
        if self.StartBtn.isEnabled():  # if app not running
            self.StartBtnFun()  # start app
        elif self.StopBtn.isEnabled():  # if app running
    ...
    def RepeatCounterFun(self):  # Repeat Radio Button Checked function
        ...
        if self.RepeatCounter == self.RepeatSpinBox.value():  # To stop
    ...
    def SettingsConfig(self):  # applies settings at startup
        ...
        self.TimeSpinBox.setValue(self.Settings.value("TimeSpinBox"))
        # set RepeatSpinBox value at startup
        self.RepeatSpinBox.setValue(self.Settings.value("RepeatSpinBox"))
        try:
    ...
    def closeEvent(self, event):  # what happend when closing the app
        ...
        self.Settings.setValue("TimeSpinBox", self.TimeSpinBox.value())
        # save RepeatSpinBox value
        self.Settings.setValue("RepeatSpinBox", self.RepeatSpinBox.value())
        try:
